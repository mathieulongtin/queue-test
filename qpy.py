...
class Client(object):
    def __init__(self,endpoint=default_endpoint):
        ...
        self._socket = self.zmq_context.socket(zmq.REQ)
        self._socket.connect(self.endpoint)

    def _send_command(self,command,*args):
        message = [command,]
        message += [ _ if _ is not None else '' for _ in args ]
        self._socket.send_multipart(message)
        results = self._socket.recv_multipart()
        if results[0] == 'ERR':
    ...
    def run_server(self):
        ...
        while True:
            if time.time() - last_check_ttr > 1:
                self.check_ttr()
            poll_result = socket.poll(timeout=1000)
            logger.debug('poll_result %s', poll_result)
            if not poll_result:
                continue
            ...
            if hasattr(self,command):
                ...
                socket.send_multipart('ERR','Unknown command "%s"' % command[0:-4])
